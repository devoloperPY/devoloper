// == JavaScript Code Start (Logika history & video BG) ==

// === Konfigurasi AI ===
const availableAIs = [ { name: "Copilot (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/copilot", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "Groq (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/groq", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "Gemini (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/gemini", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "GPT-4o (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/gpt4o", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "LetMeGPT (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/letmegpt", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "Simi (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/simi", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "Blackbox (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/blackbox", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "SmartContract (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/SmartContract", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "DegreeGuru (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/DegreeGuru", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "Llama3 (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/llama3", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "OpenAI (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/openai", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "GPT-Turbo (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/gpt-turbo", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "ChatSandbox (Mistral Large)", apiUrl: "https://api.alvianuxio.eu.org/api/chatsandbox", apiKey: "devpythonv2", method: "GET", queryParam: "text", extraParams: { model: 'mistral-large' } }, { name: "Luminai (OpenAI Prompt)", apiUrl: "https://api.alvianuxio.eu.org/api/luminai", apiKey: "devpythonv2", method: "GET", queryParam: "text", extraParams: { prompt: 'openai' } }, { name: "Deepseek (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/deepseek", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "Deepseek Coder (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/deepseek/coder", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "Deepseek R1 (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/deepseek/r1", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "Mistral (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/mistral", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "Mistral Saba (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/mistral/saba", apiKey: "devpythonv2", method: "GET", queryParam: "text" }, { name: "Mistral Moderation (Alvian)", apiUrl: "https://api.alvianuxio.eu.org/api/mistral/moderation", apiKey: "devpythonv2", method: "GET", queryParam: "text" } ];
...
let messageCountSinceLastAd = 0; // <<<<< Variabel BARU untuk menghitung pesan
const AD_TRIGGER_COUNT = 5;     // <<<<< Variabel BARU: Tampilkan iklan setiap X pesan
...
const chatVideo = document.getElementById('chat-background-video'); // Elemen Video
...
// === Fungsi sendMessageToAPI (Dengan Penambahan Logika Iklan) ===
async function sendMessageToAPI() {
    ...
    // --- Bagian Custom Response tetap sama ---
    const lowerUserText = userText.toLowerCase();
    let customResponse = null;
    const creatorKeywords = ['buat', 'cipta', 'pembuat', 'developer', 'programmer', 'bikin', 'program'];
    const whoKeywords = ['siapa', 'siapakah'];
    const youKeywords = ['kamu', 'anda', 'bot ini', 'web ini', 'aplikasi ini'];
    const askedWho = whoKeywords.some(k => lowerUserText.includes(k));
    const askedCreator = creatorKeywords.some(k => lowerUserText.includes(k));
    const askedAboutIt = youKeywords.some(k => lowerUserText.includes(k));
    if (askedWho && askedCreator && askedAboutIt) {
        ...
        setTimeout(() => {
            ...
            if (currentAI) { updateHistory(currentAI.name); }
            isSendingMessage = false;
            sendButton.disabled = false;
            userInput.disabled = false;

            // >>> CEK IKLAN SETELAH RESPONS CUSTOM <<<
            checkAndTriggerAd();
    ...
    let fetchUrl = currentAI.apiUrl;
    let fetchOptions = { method: currentAI.method, headers: { 'accept': 'application/json', ...(currentAI.headers || {}) }};
    let querySeparator = fetchUrl.includes('?') ? '&' : '?';

    // --- Logika Fetch API tetap sama ---
    if (currentAI.method === 'GET') {
        let params = {}; params[currentAI.queryParam] = originalUserText; if (currentAI.apiKey) { params['apikey'] = currentAI.apiKey; } if (currentAI.extraParams) { Object.assign(params, currentAI.extraParams); } const queryString = Object.keys(params).map(key => `${encodeURIComponent(key)}=${encodeURIComponent(params[key])}`).join('&'); fetchUrl += `${querySeparator}${queryString}`;
     } else if (currentAI.method === 'POST') {
        let bodyData = {}; bodyData[currentAI.bodyParam || 'text'] = originalUserText; fetchOptions.body = JSON.stringify(bodyData); if (!fetchOptions.headers['Content-Type']) { fetchOptions.headers['Content-Type'] = 'application/json'; }
    }
    // --- Akhir Logika Fetch API ---

    let aiResponseText = null;
    try {
        const response = await fetch(fetchUrl, fetchOptions);
        // --- Penanganan Respons API tetap sama ---
        if (!response.ok) { let errorData = null; try { errorData = await response.json(); } catch (e) { errorData = await response.text(); } console.error(`HTTP error! status: ${response.status}`, errorData); let errorMsg = `Gagal memuat data (Status: ${response.status})`; if (typeof errorData === 'string' && errorData.length < 100) { errorMsg += `: ${errorData}`; } else if (errorData?.error?.message) { errorMsg += `: ${errorData.error.message}`; } else if (errorData?.message) { errorMsg += `: ${errorData.message}`; } throw new Error(errorMsg); }
        const data = await response.json();
        let extractedResponse = null;
        if (currentAI.name === "Copilot (Alvian)" && data?.data?.response?.text) { extractedResponse = data.data.response.text; } else if (currentAI.apiUrl.includes("api.alvianuxio.eu.org") && data?.data?.response !== undefined) { if (typeof data.data.response === 'string') { extractedResponse = data.data.response; } else if (data.data.response === null) { extractedResponse = "[AI tidak memberikan respons]"; } } else if (data?.text) { extractedResponse = data.text; } else if (data?.message) { extractedResponse = data.message; } else if (data?.response && typeof data.response === 'string') { extractedResponse = data.response; } else if (typeof data === 'string') { extractedResponse = data; }
        if (typeof extractedResponse === 'string') { aiResponseText = extractedResponse; }
        else if (extractedResponse !== null && extractedResponse !== undefined) { try { aiResponseText = "[Respons Objek]: " + JSON.stringify(extractedResponse); } catch (e) { aiResponseText = "[Format respons tidak dikenal atau objek kompleks]"; } }
        else { aiResponseText = "Maaf, format respons tidak dikenal."; }
        // --- Akhir Penanganan Respons API ---

        if (typingIndicator) typingIndicator.style.display = 'none';
        displayMessage(aiResponseText, 'ai');

        // >>> CEK IKLAN SETELAH DAPAT RESPONS DARI AI <<<
        checkAndTriggerAd(); // Panggil fungsi cek iklan di sini
        ...
        // >>> CEK IKLAN SETELAH TERJADI ERROR (opsional, mungkin tidak ideal) <<<
        // checkAndTriggerAd(); // Anda bisa uncomment jika ingin tetap menghitung meski error
        ...
        userInput.disabled = false;
        isSendingMessage = false;
        if (currentAI && typeof aiResponseText === 'string') {
            updateHistory(currentAI.name);
        }
    }
}

// === Fungsi BARU untuk Cek & Pemicu Iklan ===
function checkAndTriggerAd() {
    messageCountSinceLastAd++; // Tambah hitungan pesan
    console.log(`Pesan ke-${messageCountSinceLastAd} sejak iklan terakhir.`); // Log untuk debug

    if (messageCountSinceLastAd >= AD_TRIGGER_COUNT) {
        console.log(`Mencapai batas ${AD_TRIGGER_COUNT} pesan, mencoba memicu iklan...`);

        // --- Percobaan Memicu Iklan AdSense ---
        // Ini adalah cara standar untuk meminta AdSense mengisi unit iklan.
        // Untuk Interstitial, ini SANGAT bergantung pada pengaturan Auto Ads
        // atau Anda mungkin perlu kode unit iklan Interstitial spesifik.
        try {
            // Pastikan adsbygoogle sudah diinisialisasi oleh script AdSense di <head>
...
document.addEventListener('DOMContentLoaded', () => {
    console.log(">>> DOMContentLoaded: Memanggil populateAiOptions.");
    populateAiOptions();
});
