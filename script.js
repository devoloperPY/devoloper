            ...
            tiktokV2ResultArea.innerHTML = `<div class="bg-gradient-to-br p-4">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <div class="flex flex-col space-y-2">
                        ${createShareButtonHTML({ featureId: 'tiktok-v2', url: tiktokV2UrlInput.value, title: data.title, text: "Lihat video TikTok ini!" })}
        ...
        try {
            ...
            if (data.status === true && data.result) {
                ...
                let resultHTML = downloadItems.map((item, index) => {
                    ...
                    return `<div class="bg-gradient-to-br p-4 mb-4"><p class="text-sm mb-2">by: ${item.username || 'Unknown'}</p>${mediaTag}<p class="text-sm mb-4">${item.caption || ''}</p><div class="flex items-center space-x-2"><button data-url="${downloadUrl}" data-filename="${finalFilename}" class="download-button-ig flex-1 mt-2 text-center bg-pink-600 text-white font-semibold py-2 px-4 rounded-lg"><i class="fas fa-download mr-2"></i>Download (.${fileType})</button><button class="add-to-queue-button" data-url="${downloadUrl}" data-filename="${finalFilename}"><i class="fas fa-plus"></i></button>${createShareButtonHTML({ featureId: 'instagram-vintex', url: postUrl, title: item.caption || 'Postingan Instagram' })}</div></div>`;
                }).join('');
