...
// === Fungsi Riwayat ===
function getHistory() { try { const historyJson = localStorage.getItem(HISTORY_KEY); return historyJson ? JSON.parse(historyJson) : []; } catch (e) { console.error("Gagal membaca riwayat:", e); return []; } } function saveHistory(historyData) { try { if (historyData.length > MAX_HISTORY_ITEMS) { historyData = historyData.slice(historyData.length - MAX_HISTORY_ITEMS); } localStorage.setItem(HISTORY_KEY, JSON.stringify(historyData)); } catch (e) { console.error("Gagal menyimpan riwayat:", e); } } // Update: slice dari belakang agar yg terbaru yg tersimpan jika > MAX
function updateHistory(aiName) { if (!aiName || currentChatMessages.length === 0) { return; } const history = getHistory(); const now = Date.now(); const historyId = `${aiName}-${now}-${Math.random().toString(16).slice(2)}`; const messagesSnapshot = [...currentChatMessages]; const newEntry = { id: historyId, aiName: aiName, timestamp: now, messages: messagesSnapshot }; let updatedHistory = history.filter(item => item.aiName !== aiName); // Hapus entri lama dgn AI sama (jika ada)
 updatedHistory.unshift(newEntry); // Tambah entri baru di awal
 saveHistory(updatedHistory); // Simpan, fungsi saveHistory akan memotong jika > MAX
} function displayHistoryList() { const history = getHistory(); historyList.innerHTML = ''; if (history.length === 0) { historyList.innerHTML = '<li>Belum ada riwayat chat.</li>'; return; } history.sort((a,b)=> b.timestamp - a.timestamp); history.forEach(item => { const li = document.createElement('li'); li.dataset.historyId = item.id; const date = new Date(item.timestamp); const formattedDate = date.toLocaleString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' }); const lastMsg = item.messages && item.messages.length > 0 ? item.messages[item.messages.length - 1] : null; const secondLastMsg = item.messages && item.messages.length > 1 ? item.messages[item.messages.length - 2] : null; const previewUser = secondLastMsg?.sender === 'user' ? secondLastMsg.text : (lastMsg?.sender === 'user' ? lastMsg.text : '...'); const previewAi = lastMsg?.sender === 'ai' ? String(lastMsg.text).split('\n')[0].replace(/<[^>]*>/g, '').substring(0, 80) : '...'; // Ambil baris pertama & potong
...
async function sendMessageToAPI() {
    ...
    try {
        ...
        if (data?.data?.response?.text && typeof data.data.response.text === 'string') {
            // Prioritas utama: format Alvianuxio { data: { response: { text: "..." } } }
            ...
            // Fallback jika data.data adalah string
            extractedResponse = data.data;
        } else if (typeof data === 'string') {
             // Fallback jika responsnya memang hanya string
        ...
        if (typeof extractedResponse === 'string') {
        ...
        } else if (data !== null && data !== undefined) { // Fallback [Respons Objek] diperbaiki agar tidak mudah terpicu
            // Hanya tampilkan [Respons Objek] jika benar2 tidak ada teks yg bisa diekstrak
            ...
            aiResponseText = "[Respons AI kosong atau tidak dikenal]"; // Pesan lebih jelas
        }
